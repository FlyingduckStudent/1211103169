...
//* This is for saving game data to a txt file *

void savegame(vector<vector<string>> board, vector<Zombies> zombie, int currentHealth, int alienattack) {
  ofstream myfile;
  myfile.open("gameboard.txt");
  //* save map *
  //board dimensions //myfile << board.size() << " " << board[0].size() << endl;
  ...
  //* save alien data *
  myfile << currentHealth << " " << alienattack << endl;
  //* save zombie data *
  //number of zombies //myfile << zombie.size() << endl;
  for (int i = 0; i < zombie.size(); i++) {
    myfile << zombie[i].number << " " << zombie[i].life << " " << zombie[i].attack << " " << zombie[i].range << endl;
  }
  myfile.close();
}

//* This is for loading data from a txt file *
loadgame() {
vector<vector<string>> gameboard;
vector<Zombies> zombie;

int rows, columns, currentHealth, alienattack, amountOfZombies;
...
if (file.is_open()) {
  //* load map *
  file >> rows >> columns;
  gameboard.resize(rows, vector<string>(columns));
  ...
  //* load alien stats *
  file >> currentHealth >> alienattack;

  //* load zombies and stats *
  file >> amountOfZombies;
  zombie.resize(amountOfZombies);
  for (int i = 0; i < amountOfZombies; i++) {
    file >> zombie[i].number >> zombie[i].life >> zombie[i].range >> zombie[i].attack >> zombie[i].coordinates;
  }
...
void alienmove(vector<vector<string>> &board, vector<Zombies> &zombies)
{
    ...
    cout << "Alien : Life " << currentHealth << ", Attack " << alienattack << endl;
    ...
    while (true)
    {
        ...
        else if (direction == "quit")
        {
            ...
            {
     // Save to txt file
     savegame(board,zombies,currentHealth,alienattack);
        }
